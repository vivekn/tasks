...
def add_jobs(file_name):
	for line in open(file_name):
		key = line.strip()
		if (not db.sismember(ns_in_progress, key)) and (not db.sismember(ns_completed, key)):
			db.sadd(ns_jobs, key)
...
def help():
	...
	print "\n    run -> start processing the jobs (with resume support)"
	print "\n    status -> show the current status of jobs in progress"
	print "\n    reset -> reset all jobs and logs"
